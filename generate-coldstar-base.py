#!/usr/bin/env python3
"""Generate Coldstar for Base demo voiceover using Matthew's cloned voice.

2-3 minute narration for Base Batches 003 application video.
...
# ── Voiceover Segments ──
# ~2.5 minutes total at natural pace
# Each segment: 15-25 seconds of speech

SEGMENTS = [
    ("s01-intro",
     "This is Coldstar. An air-gapped cold wallet. Now on Base."),
    ...
    ("s04-how-it-works",
     ...
     "Amount. Recipient. Gas fees. All fetched from Base's RPC. "
     ...
     "Scan it on the online device. Broadcast to Base. Done."),
    ...
    ("s06-base-native",
     "We built this specifically for Base. Native secp256k1 ECDSA signing. "
     "EIP 1559 Type 2 transactions. Base Mainnet and Sepolia testnet support. "
     "This isn't a generic EVM wallet that happens to work on Base. "
     "It's built for Base from the ground up."),

    ("s07-mcp-server",
     "Coldstar includes an MCP server with five tools for Base. "
     "ETH and ERC-20 balances. Gas price estimation. Token prices via Pyth oracle. "
     "Full portfolio tracking with USD values. "
    ...
    ("s11-cta",
     "Coldstar. Cold signing for Base. Check it out at coldstar dot dev."),
]
...
def generate_silence(duration_sec, output_path, sample_rate=24000):
    """Generate a silence WAV file using ffmpeg."""
    cmd = [
        "ffmpeg", "-y", "-f", "lavfi",
...
def main():
    ...
    # Generate 0.8s silence for gaps between segments
    ...
    wavs = sorted([
        ...
        if f.startswith("s0") and f.endswith(".wav")
    ])
    ...
    # Output as WAV first (for video compositing)
    output_wav = os.path.join(OUTPUT_DIR, "coldstar-base-voiceover.wav")
    cmd = [
    ...
    # Also output as M4A
    output_m4a = os.path.join(OUTPUT_DIR, "coldstar-base-voiceover.m4a")
    cmd = [
    ...
    desktop_copy = os.path.expanduser("~/Desktop/coldstar-base-voiceover.m4a")
