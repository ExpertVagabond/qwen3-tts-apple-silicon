#!/usr/bin/env python3
"""Batch generate Schneider interview answers using Qwen3-TTS MLX."""
...
OUTPUT_DIR = os.path.expanduser("~/Desktop/schneider-qwen-tts")
SPEAKER = "Ryan"  # Male English — confident, natural
SPEED = 0.95  # Slightly slower for interview pacing
EMOTION = "Confident and articulate, speaking at a steady conversational pace"
...
def main():
    ...
    # Load CustomVoice model (has speaker presets + emotion control)
    model_folder = "Qwen3-TTS-12Hz-1.7B-CustomVoice-8bit"
    ...
    # Concatenate each question's segments into a single WAV
    ...
    # Concatenate ALL into one full practice file
    ...
    if all_m4as:
        ...
        full_output = os.path.join(OUTPUT_DIR, "schneider-full-practice-qwen.m4a")
        cmd = [
            "ffmpeg", "-y", "-f", "concat", "-safe", "0",
            "-i", full_concat,
